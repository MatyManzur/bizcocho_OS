    ...
    while (1)
    {
        ...
        if (!error)
        {
            if (IS_PIPE(pipeTokenCount))
            {
                ...
                uint32_t leftPid = executeNonBuiltIn(commands[foundCommand[0]].name, commands[foundCommand[0]].programFunction, argc[0] - leftBackground, (void **)argv[0], leftBackground ? EMPTY : NO_CHANGE_FD, writeFd, leftBackground);
                if (leftPid == 0)
                    continue;
