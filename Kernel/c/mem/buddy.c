...
uint8_t logBase2(uint32_t nbytes, uint8_t ceiling)
{
    if (nbytes == 0)
        return 0;
    uint8_t value = 0;
    while (ceiling ? TWO_TO_POWER_OF(value) < nbytes : TWO_TO_POWER_OF(value + 1) <= nbytes)
    {
        value++;
    }
    return value;
...
void memInitialize(void *memBase, uint32_t memSize)
{
    ...
    myBase->memSize = TWO_TO_POWER_OF(myBase->largestDivision);
    memUsed = 0;
...
static void *updatingLists(uint8_t division)
{
    ...
    uint32_t size = TWO_TO_POWER_OF(division);
